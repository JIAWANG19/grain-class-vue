<template>
    <div class="app-container">
        ...
        <!-- 分页组件 -->
        <el-pagination :current-page="pageNo" :total="total" :page-size="pageSize" :page-sizes="[5, 10, 20, 30, 40, 50, 100]"
    ...
    methods: {
        ...
        batchRemove() {
            ...
            if (this.multipleSelection.length == 0) {
                this.$message.warning('没有任何数据被选中')
                return
            }
            this.$confirm('此操作将永久删除所有被选中的讲师信息, 是否继续?', '提示', {
        ...
        // 条件分页查询
        fetchData() {
            teacherApi.pageList(this.pageNo, this.pageSize, this.searchObj)
                .then(response => {
        ...
        removeById(id) {
            this.$confirm('此操作将永久删除该讲师的信息, 是否继续?', '提示', {
                ...
                teacherApi.remoreTeacherById(id)
                    .then((response) => {
                        this.$message({
                            type: 'success',
                            message: '删除成功'
